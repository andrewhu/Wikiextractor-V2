...
import time
import babel
from bs4 import BeautifulSoup # Needed when some articles have bad wiki syntax
from pylatexenc.latex2text import LatexNodes2Text as  LatexNodes2Text # to compile latex text
import warnings
#import os
#from itertools import zip_longest


#getting rid of several libraries unnecessary warning messages    
...
acceptedNamespaces = ['w']

##
# Do not expand unwanted templates
notExpandTemplates = [  """'""", 
                        r'^\s+$', # white spaces
                        '!',
                        'ISBN', 
                        ...
                        'Pie chart',
                        'Graph',
                        r'(.*)Infobox(.*)',  #too complicated to parse, program fails
                        r'(.*)Infotable(.*)',   # #too complicated to parse, program fails,
                        r'(.*)Image(.*)',   # #too complicated to parse, program fails
                        'Flag',
                        'pdf',
                        'increase',
                        'decrease',
...
notExpandTemplates = re.compile('|'.join(notExpandTemplates), re.IGNORECASE)


##
# Aux vars to some templates: do not touch this!

# Coord templates formating
_aux_template_coord_aux_var = ["""º""", """'""", """''""" ]
...
def clean(extractor, text, expand_templates=False, language = None, html_safe=True):
    ...
    :param language: only used if expand_templates is True
    ...
    # Links are processed after html elements
    ...
    # drop empty parenthésis
    #text = re.sub( r'\(\s*\)', '' , text) # dont use this one since code like main() will be deleted
    ...
    # Process <chem> and <code>   tags
    text = re.sub('(&lt;)(chem|code)(.*?)(&gt;)(.*?)(&lt;/)(chem|code)(&gt;)', r'\5', text, flags=re.DOTALL ) 

    # Process <math> tags
    try:
        text = re.sub('(&lt;)(math)(.*?)(&gt;)(.*?)(&lt;/)(math)(&gt;)', lambda x: LatexNodes2Text().latex_to_text(x.group(5)), text, flags=re.DOTALL ) 
    except:
        pass #ignore problemas
    ...
    #############################################
    ## done after cleaning &lt; refs and other html elements
    #   since sometimes leaks trash text
    ...
    ##
    #  Reclean links (for some bad formated chunks)
...
class Extractor():
    ...
    def extract(self, out, html_safe=True):
        ...
        if(text is None):
            logging.debug('\t\t\t discarding doc. with title:' + self.title +' since it  contains a banned template specified in config.')
            return False #doc not written
        ...
        if self.to_json:
            ...
            if(self.generator): #yield doc
            ...
            else: #write in file the doc

                # Saving doc (chunks of N size)
                ...
                # Single file
                #output_name = str(self.id) + '.json'
                #out = os.path.join(out, output_name)
            ...
            if(self.generator): #yield doc
                ...
                ## SINGLE FILE PER DOC
                #output_name = str(self.id)+ '.txt'
                #out = os.path.join(out, output_name)
                #with open(out, 'w', encoding='utf8') as output:
                #    #left  "\n" so that the whole doc separate paragraphs
                #    output.write("\n".join(text)) 
    ...
    def expandTemplate(self, body, language = None):
        ...
        if(title is None): 
            ...
            #   Recursion
            return None
        elif(re.match(notExpandTemplates,title) ):
            ## """'""" : Empty template name. Sometimes is a table 
        ...
        ## Handling lang templates
        # Some templates like lang| or lang-** are included in wikidumps
    ...
    if functions:
        ...
        if funct:
            ...
            if not templateTitle:
                ...
                logging.debug("Template with empty title")
...
def define_template(title, page):
    ...
    if text:
        if title in templates and templates[title] != text:
            ...
            logging.debug('Redefining: %s', title)
